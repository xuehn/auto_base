...
module.exports = (postMessageToWebView) => {
  ...
  let bridgeHandler = {
    ...
    mainRun: (business_type, business_type_name) => {
        if(business_type){
            log('即将从ui启动脚本，更新本次业务类型=>【'+business_type_name+'】：'+business_type);
            bridgeHandler.saveConfigs({
                business_last_run_business_type: business_type,
                business_last_run_business_type_name: business_type_name,
            });
        }else {
            log('即将从ui启动脚本，沿用上次业务类型=>【'+business_type_name+'】：'+config.business_last_run_business_type);
        }
        ui.run(function () {
            myEngines.execScriptFile(FileUtils.getCurrentWorkPath() + '/mainRun.js', { path: FileUtils.getCurrentWorkPath() })
        })
    }
  }
